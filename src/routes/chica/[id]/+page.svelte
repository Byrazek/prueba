  ...
  async function busqueda(param) {
    try {
      const res = await fetch(API_URL + '/api/product', {
        ...
        body: JSON.stringify({id: param, table: 'chica'})
      });
      if (!res.ok) {
    ...
    <section class="container mx-auto my-36 px-4 md:px-6 py-8">
      <div class="grid lg:grid-cols-2 gap-6 items-start">
        <div>
          ...
          <img src="{images[imageName]}" alt="{product.description}" class="rounded-lg object-cover w-full aspect-square" style="width: 600px; height: 600px;"/>
        </div>
        <div>
